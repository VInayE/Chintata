    ...
    function BtnDelete(button)
{
  ...
  if (result === true){
    ...
    fsi.workflow('Delete Issue', inputs, outputs, null, 
                 function(responseData){fsi.changePage('MyTimeline');
...
function AddIssue(liked,status,userImage, userHandle, points, text, time, privacy, noLikes, noComments, systemId,image, userSystemId){
  ...
  var issue = 
      '<div class="swiper-slide">' +
        '<!-- Card Projects -->' +
          '<div class="col-md-6 col-md-offset-3" style="width:98%;">' +
...
function MyCalls(){
 ...
 swiper.removeAllSlides(); 
  ...
  filterDelete = 'MyCalls';
  fsi.setById('txtFILTER',filterDelete);
...
function onLoad(){
  ...
  if($("[data-control-id='filteroptions']").hasClass("filter-options-show"))
  {
    ...
    // Set search width
     $("[data-control-id='containerSearch']").width = window.innerWidth - 200;
